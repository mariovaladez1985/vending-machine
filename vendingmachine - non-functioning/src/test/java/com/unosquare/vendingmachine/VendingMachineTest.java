...
public class VendingMachineTest {
	...
	public void addCashBalanceInvalid() throws Exception {
		mockMvc.perform(get("add-to-balance").param("denomination", "NOTE_5")).andExpect(status().isUnauthorized())
				.andExpect(content()
	...
	public void refundBalanceWithPurchase() throws Exception {
		...
		if (lstLineas.size() > 0) {
			...
			mockMvc.perform(get("/end-transaction").session(mockHttpSession))
			.andExpect(status().isOk())
